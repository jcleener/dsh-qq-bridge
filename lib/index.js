...
export function apply(ctx) {
	...
	async function cmdStatus() {
		...
		lines.push('', '指令: /profile /会话 /会话 <ID> /new /实例 <端口> /状态 /帮助');
		return lines.join('\n');
	}
	const CMD_HELP = [
		'QQ-DSH 桥接（单插件版）',
	...
	ctx.inject(['settings', 'sessionController', 'sessionQuery', 'sessions', 'approval', 'webServer'], (host) => {
		...
		log(`已加载（v0.8.0, web:${webPort ?? '?'}）。到设置页 → QQ 桥接，填写凭证并打开"启用"。`);
		try {
			svc.webServer.register({
				...
				handler: async (req, res) => {
					...
					if (req.method === 'POST') {
						...
						const patch = {};
						for (const key of ['enabled', 'sandbox', 'steer', 'compact']) {
							if (typeof input[key] === 'boolean') patch[key] = input[key];
						}
						if (typeof input.appId === 'string') patch.appId = input.appId;
						if (typeof input.createCwd === 'string') patch.createCwd = input.createCwd;
